...
const app = express();
// Instantiate PrismaClient correctly
...
app.use(cors()); 
// Increase limit to handle potential base64 attachments
app.use(express.json({ limit: '10mb' })); 

// --- Helper Functions ---
const seedDatabase = (): Promise<string> => {
    return new Promise((resolve, reject) => {
        const command = 'npx prisma db seed';
        exec(command, (error, stdout, stderr) => {
            if (error) {
                console.error(`Seed error: ${error.message}`);
                return reject(new Error('Failed to seed database.'));
            }
            if (stderr) {
