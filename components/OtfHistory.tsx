...
import type { User, OTF_History_Log } from '../types';
import { ClockIcon } from './icons/Icons';

interface OtfHistoryProps {
    currentUser: User;
...
export const OtfHistory: React.FC<OtfHistoryProps> = ({ currentUser, otfHistory, users }) => {
    ...
    const sortedHistory = useMemo(() => {
        return [...otfHistory].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    }, [otfHistory]);

    return (
        <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-800">OTF Action History</h2>
            </div>
                ...
                <table className="w-full text-sm text-left text-gray-500">
                        ...
                        {sortedHistory.map(log => {
                            ...
                            return (
                                <tr key={log.id} className="bg-white border-b hover:bg-gray-50">
                                    <td className="px-6 py-4 font-mono font-semibold text-gray-700">{log.otfIdString}</td>
                 ...
                 {sortedHistory.length === 0 && (
                    <div className="text-center py-12 text-gray-500">
                        ...
                        <p>No OTF actions have been recorded yet.</p>
                    </div>
                )}
            </div>
        </div>
