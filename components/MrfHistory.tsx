...
import type { User, MRF_History_Log } from '../types';
import { ClockIcon } from './icons/Icons';

interface MrfHistoryProps {
    currentUser: User;
    mrfHistory: MRF_History_Log[];
    users: User[];
}

const getActionBadge = (action: 'Created' | 'Approved' | 'Rejected') => {
...
export const MrfHistory: React.FC<MrfHistoryProps> = ({ currentUser, mrfHistory, users }) => {
    ...
    const sortedHistory = useMemo(() => {
        return [...mrfHistory].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    }, [mrfHistory]);

    return (
        <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-800">MRF Action History</h2>
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left text-gray-500">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            <th scope="col" className="px-6 py-3">MRF ID</th>
                        ...
                        {sortedHistory.map(log => {
                            ...
                            return (
                                <tr key={log.id} className="bg-white border-b hover:bg-gray-50">
                                    <td className="px-6 py-4 font-mono font-semibold text-gray-700">{log.mrfIdString}</td>
                 ...
                 {sortedHistory.length === 0 && (
                    <div className="text-center py-12 text-gray-500">
                        ...
                        <p>No MRF actions have been recorded yet</p>
                    </div>
                )}
            </div>
        </div>
